...
\subsubsection{Objects with methods and private members using hashes and closures}
...
\todo{This needs much mroe explanation of what namespaces and that actually are}
\label{packages}
...
\subsection{Automatically imported packages}
All the functions (i.e. words which do not compile directly to bytecode) 
mentioned so far -- \texttt{p}, \texttt{i}, \texttt{quit} and so on -- belong to a number
of special packages which are automatically imported when Angort starts. These include
\texttt{coll} for functions which handle collections, and \texttt{std} for unclassified functions.
For example, the \texttt{quit} function has the fully qualified name \texttt{std\$quit}.
User-defined functions are added to the \texttt{user} namespace.

Although they are defined as constants, it possible to override these functions. For example the standard function \texttt{p}
